...
export async function retrieveSupportChunks({
  ...
  if (new Set(tenantChunks.map((chunk) => chunk.sourceId)).size > 1) {
    const selected = []
    const seen = new Set()
    ...
    for (const chunk of reranked) {
      ...
      if (selected.length >= Math.min(k, tenantChunks.length)) break
    }

    const remaining = reranked.filter((chunk) => !selected.some((item) => item.id === chunk.id))
    return [...selected, ...remaining].slice(0, k)
  }
